...
pub fn build_editor(buffer: String, line: u32, column: u32) -> Result<Editor, String> {
...
mod tests {
    ...
    fn test_build_editor() {
        ...
        let editor = build_editor(String::from(buffer), 10, 10);
    ...
    fn test_line_count() {
        let buffer = "Hello, world!\nThe 2nd line.";
        let editor = build_editor(String::from(buffer), 0, 0).unwrap();
        assert_eq!(editor.line_count(), 2);

        let editor = build_editor(String::new(), 0, 0).unwrap();
        assert_eq!(editor.line_count(), 1);

        let editor = build_editor(String::from("aaa bbb"), 0, 0).unwrap();
        assert_eq!(editor.line_count(), 1);
    }
    ...
    fn test_move_right() {
        let buffer = "Hello, world!\nThe 2nd line.";
        let mut editor: Editor = build_editor(String::from(buffer), 1, 6).unwrap();
        let expected = [7, 8, 9, 10, 11, 12, 13, 13];
        for i in 0..expected.len() {
            editor.move_right(1);
            assert_eq!(editor, build_editor(String::from(buffer), 1, expected[i]).unwrap());
        }

        for i in 0..editor.line_width(editor.line()).unwrap() {
            let mut editor = build_editor(String::from(buffer), 1, i as u32).unwrap();
            let width = editor.line_width(editor.line()).unwrap();
            editor.move_right(width + 1);
            assert_eq!(editor, build_editor(String::from(buffer), 1, width).unwrap());
        }
    }
    ...
    fn test_move_left() {
        let buffer = "Hello, world!\nThe 2nd line.";
        let mut editor = build_editor(String::from(buffer), 1, 6).unwrap();
        ...
        for i in 0..editor.line_width(editor.line()).unwrap() {
            ...
            assert_eq!(editor, build_editor(String::from(buffer), 1, 0).unwrap());
        }
    }
    ...
    fn test_move_up() {
        let buffer = "Hello, world!\nThe 2nd line.\nAAABBBCCC.";
        let mut editor = build_editor(String::from(buffer), 2, 4).unwrap();
        let expected = [1, 0, 0];
        for i in 0..expected.len() {
            editor.move_up(1);
            assert_eq!(editor, build_editor(String::from(buffer), expected[i], 4).unwrap());
        }

        for i in 0..editor.line_count() {
            let mut editor = build_editor(String::from(buffer), i as u32, 1).unwrap();
            let count = editor.line_count();
            editor.move_up(count);
            assert_eq!(editor, build_editor(String::from(buffer), 0, 1).unwrap());
        }
    }
    ...
    fn test_move_down() {
        let buffer = "Hello, world!\nThe 2nd line.\nAAABBBCCC.";
        let mut editor = build_editor(String::from(buffer), 0, 4).unwrap();
        let expected = [1, 2, 2];
        for i in 0..expected.len() {
            editor.move_down(1);
            assert_eq!(editor, build_editor(String::from(buffer), expected[i], 4).unwrap());
    ...
    fn test_insert_at() {
        let buffer = "Hello, world!\nThe 2nd line.\nAAABBBCCC.";
        let mut editor = build_editor(String::from(buffer), 0, 6).unwrap();
        editor.insert_at('\n', 0, 6);
        assert_eq!(
            editor,
            build_editor(
    ...
    fn test_delete_at() {
        ...
        assert_eq!(editor, build_editor(String::from("abcdef"), 0, 3).unwrap());
        editor.delete_at(10, 10);
        assert_eq!(editor, build_editor(String::from("abcdef"), 0, 3).unwrap());
        editor.delete_at(0, 1);
        assert_eq!(editor, build_editor(String::from("acdef"), 0, 2).unwrap());
    }
}
